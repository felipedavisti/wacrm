...
interface Script {
  config?: { id: string; user_id: string } | null; // whatsapp_config .limit(1)
...
function makeDb(script: Script): SupabaseClient {
  ...
  const builder: Record<string, unknown> = {
    ...
    order: () => builder,
    limit: () => {
      // whatsapp_config: both the config check and resolveAuditUserId now
      // terminate on `.limit(1)` (spec 007) → array.
...
describe('resolveConversationByPhone', () => {
  ...
  it('returns the existing contact + conversation without creating', async () => {
    const db = makeDb({
      config: { id: 'wc-1', user_id: 'owner-1' },
